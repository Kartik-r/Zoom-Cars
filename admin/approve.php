...
<h2 style="text-align: center;font-size: 35px;color:black; text-decoration: underline; ">
    ...
    <form class="Approve" method="post" action="" >
        <input type="text" name="approve" class="form-control" placeholder="Approve or not" required="">
        <input type="date" name="issue" class="form-control" placeholder="Issue Date " required="">
        <input type="date" name="return" class="form-control" placeholder="Return Date" required="">
       
        <input type="text" name="reg" class="form-control" placeholder="Car Registration Number" required=""><br>
...
{
    mysqli_query($db,"UPDATE `issue_car` SET `approve` =
    '$_POST[approve]', `issue`='$_POST[issue]',
    `return` ='$_POST[return]' WHERE username='$_SESSION[name]' AND 
    reg='$_SESSION[reg]';");

    mysqli_query($db,"UPDATE `car` SET quantity = 
    quantity-1 where reg='$_SESSION[reg]';");

    $res=mysqli_query($db,"SELECT quantity FROM `car` 
    WHERE reg='$_SESSION[reg]';");

while($row=mysqli_fetch_assoc($res))
{
    if($row['quantity']==0)
    {
        mysqli_query($db,"UPDATE `car` SET status = 
        'not-available' where reg='$_SESSION[reg]';");
    }
}?>
